...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function obj = GUI()
            ...
            % Create the window
            obj.app.figure   = uifigure('Name','PlatEMO v4.5','Position',[0 0 1200 650],'Interruptible','off','icon',obj.icon.logo1,'BusyAction','cancel','Visible','off','WindowButtonMotionFcn',@(~,~)[]);
	...
	methods(Access = private)
        ...
        function delete(obj)
            imagetime = clock;
            if ~isfield(obj.icon,'imagetime') || etime(imagetime,obj.icon.imagetime) > 86400
                try
