function varargout = caculateDistance(objs)
...
%--------------------------------------------------------------------------
    ...
    K = size(objs,1); % K 个解
    fmax = max(objs); % m 个目标解的最大值
    fmin = min(objs); % m 个目标解的最小值
    L = ceil((objs-fmin)./(fmax-fmin)); % 对每个目标进行归一化,ceil 进行四舍五入
    ...
    PCD = reshape(PCD,K-1,K)'; % 得到每个非支配解与其他非支配解的距离，每个非支配解对应一行
