...
%--------------------------------------------------------------------------
    ...
    methods
        function main(Algorithm,Problem)
            addpath("D:\Code\PlatEMO\PlatEMO\UserAlgorithms\ComparePccs");
            %% Generate random population
            Population       = Problem.Initialization(); % 初始化种群
            Pbest            = Population;  % 初始话 Pbest
            [Gbest, CrowdDis, density]  = useDistance(Population,Problem.N);
            ...
            while Algorithm.NotTerminated(Gbest)
                ...
                [Gbest, CrowdDis, density] = useDistance([Gbest,Population],Problem.N); % 利用 PCCS 进行 Gbest 选择
